...
namespace SalesManagementSystem
{
    public partial class IssueListForm : Form
    {
        ...
        {
            AC.sql = "select ss.出庫ID, od.発注ID, od.商品ID, pd.商品名, od.発注数量, mk.メーカー名, ff.営業所名, ss.ステータス from (((出庫テーブル as ss inner join 発注テーブル as od on ss.発注ID = od.発注ID) inner join 商品マスタ as pd on od.商品ID = pd.商品ID) inner join 仕入先マスタ as mk on ss.メーカー名 = mk.メーカー名) inner join 営業所マスタ as ff on ss.営業所ID = ff.営業所ID;";
